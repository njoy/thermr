...
.. glossary::
    nendf 
        ...
        This is the thermal scattering input, which is the output from LEAPR. nendf can be set to zero if no LEAPR run is performed prior to this calculation, but doing so does limit THERMR's capabilities. In the directory that NJOY is run, nendf should appear as "tape24"

    nin

        This is the input PENDF file, which can be obtained from (for example) RECONR and BROADR. This should be available in the NJOY bin directory as "tape23".

    nout

        This is the file to which THERMR will write the final PENDF. The final output PENDF will be saved as "tape25".
