  ...
  // ----- have something else to do before then
   ...
   if ( x[i-1] != xl ){
     ...
     gral+= 0.5*(yl*x[i-1]-y[i-1]*xl)*(x[i-1]+xl)
       +(1.0/3.0)*(y[i-1]-yl)*(x[i-1]*x[i-1]+x[i-1]*xl+xl*xl);
   }
...
auto sigl( double e, double ep, double tev, std::vector<double> alpha,
  ...
  */
  ...
  std::vector<double> x(imax), y(imax),p(nlin);
   ...
   // constant factors
   nl = std::abs(nlin);
   b = ( lat == 1 and iinc == 2 ) ? std::abs( (ep-e) / tevz ) :
                                    std::abs( (ep-e) / tev  );
   ...
   x[0] = 1;
   x[1] = (ep == 0) ? 0 : ( e + ep - (sqrt(1+b*b)-1) * az * tev ) / 
                          ( 2 * sqrt(e*ep) );
   ...
   ymax =  std::max(y[0],std::max(y[1],std::max(y[2],eps))); 
  ...
  // if (i == imax) go to 120
   while (true){ 
     while ( true ){
       ...
       test = 0.5 * tolin * ( std::abs(yt) + ymax / 50.0 );
       test2 = ym + ymax / 100;
       if ( ( std::abs(yt-ym)         <= test  and 
              std::abs(y[i-2]-y[i-1]) <= test2 and 
              (x[i-2]-x[i-1]) < 0.5 ) or 
            ( x[i-2] - x[i-1] < xtol ) ){
         break;
       }
   ...
   x[0] = 1;
   x[1] = ep == 0 ? 0 : 0.5*(e+ep-(sqrt(1+b*b)-1)*az*tev)/sqrt(e*ep);
